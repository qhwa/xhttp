...
export function xhttp (params, onSuccess, onError) {
  ...
  xhr.send(serialiseBody(method, params.body, headers))
}
...
function serialiseBody (method, body, headers) {
  return readOnly(method)
    ? null
    : needSerialise(headers) && isSerialisable(body)
    ? JSON.stringify(body)
    : body
}
...
function needSerialise (headers) {
  return isUrlencodedForm(headers);
}

function isUrlencodedForm(headers) {
